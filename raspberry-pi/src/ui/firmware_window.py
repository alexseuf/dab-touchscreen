...
class FirmwareMainWindow(MainWindow):
    ...
    def _firmware_page(self):
        ...
        self._add_firmware_row(grid, 0, "Aktuelle Version", self.fw_current); self._add_firmware_row(grid, 1, "Repository", self.fw_repository)
        check_row = QtWidgets.QWidget(); check_layout = QtWidgets.QHBoxLayout(check_row); check_layout.setContentsMargins(0, 0, 0, 0); check_layout.setSpacing(5); check_layout.addWidget(self.fw_last_check, 3)
        self.fw_check_button = QtWidgets.QPushButton("Jetzt prüfen"); self.fw_check_button.setMinimumHeight(38); self.fw_check_button.clicked.connect(self._check_firmware_versions); check_layout.addWidget(self.fw_check_button, 2)
        self._add_firmware_row(grid, 2, "Letzte Prüfung", check_row)
        self._add_firmware_row(grid, 3, "Neueste Version", self.fw_latest); self._add_firmware_row(grid, 4, "Status", self.fw_status)

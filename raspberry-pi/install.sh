...
# Persistent I2C + DS3231 kernel RTC. Safe if the RTC is temporarily absent.
BOOT_CONFIG=/boot/firmware/config.txt
[[ -f "$BOOT_CONFIG" ]] || BOOT_CONFIG=/boot/config.txt
if [[ -f "$BOOT_CONFIG" ]]; then
 sed -i -E '/^[[:space:]]*dtparam[[:space:]]*=[[:space:]]*i2c_arm[[:space:]]*=/d; /^[[:space:]]*dtoverlay[[:space:]]*=[[:space:]]*i2c-rtc,ds3231[[:space:]]*$/d' "$BOOT_CONFIG"
 printf '\n# DAB Touchscreen: I2C + DS3231 hardware RTC\ndtparam=i2c_arm=on\ndtoverlay=i2c-rtc,ds3231\n' >>"$BOOT_CONFIG"
 modprobe i2c-dev 2>/dev/null || true
else
 echo "WARNUNG: Raspberry-Pi-Bootkonfiguration nicht gefunden; RTC konnte nicht konfiguriert werden." >&2
fi
...
DAB_ENABLE_SIMULATOR=0; DAB_MQTT_LISTEN_ADDRESS=0.0.0.0; DAB_MQTT_USERNAME=; DAB_MQTT_PASSWORD=
...
PY
for value in "${DAB_MQTT_USERNAME:-}" "${DAB_MQTT_PASSWORD:-}"; do if [[ -n $value && ! $value =~ ^[A-Za-z0-9._@%+=:-]+$ ]]; then echo "MQTT-Zugangsdaten dürfen nur A-Z, a-z, 0-9 und ._@%+=:- enthalten" >&2; exit 1; fi; done
...
else ALLOW_ANONYMOUS=true; PASSWORD_DIRECTIVE=; rm -f /etc/mosquitto/dab-touchscreen.passwd; fi
umask 027
{ printf 'DAB_ENABLE_SIMULATOR=0\n'; printf 'DAB_MQTT_LISTEN_ADDRESS=%s\n' "${DAB_MQTT_LISTEN_ADDRESS:-0.0.0.0}"; printf 'DAB_MQTT_USERNAME=%s\n' "${DAB_MQTT_USERNAME:-}"; printf 'DAB_MQTT_PASSWORD=%s\n' "${DAB_MQTT_PASSWORD:-}"; } >"$ENV_DIR/env"
...
install -o root -g root -m 0644 "$ROOT_DIR/systemd/dab-rtc-sync.service" /etc/systemd/system/dab-rtc-sync.service
...
for desktop in lxpolkit-notify.desktop nm-applet.desktop; do cat >"/home/$SERVICE_USER/.config/autostart/$desktop" <<EOF
[Desktop Entry]
Type=Application
Name=DAB kiosk suppression
Hidden=true
EOF
chown "$SERVICE_USER:$SERVICE_USER" "/home/$SERVICE_USER/.config/autostart/$desktop"; done
...
systemctl enable NetworkManager.service mosquitto.service lightdm.service dab-rtc-sync.service
...
echo "RTC: DS3231 konfiguriert; Neustart erforderlich, damit der Kernel-Treiber aktiv wird."
